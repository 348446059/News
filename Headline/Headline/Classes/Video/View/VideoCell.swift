...
class VideoCell: UITableViewCell,RegisterCellFromNib {
    ...
    override func awakeFromNib() {
        ...
        nameLabel.theme_textColor  = "colors.moreLoginTextColor"
        ...
        concernButton.theme_setTitleColor("color.black", forState: .normal)
        concernButton.theme_setTitleColor("colors.grayColor210", forState: .normal)
    ...
    /// 关注点击
    @IBAction func concernButtonCilcked(_ sender: UIButton) {
        if sender.isSelected {
            // 已关注用户，取消关注
            NetworkTool.loadRelationUnfollow(userId:video.user_info.user_id) { (_) in
                sender.isSelected = !sender.isSelected
            }
        }else {
            NetworkTool.loadRelationFollow(userId: video.user_info.user_id) { (_) in
                sender.isSelected = !sender.isSelected
            }
        }
    }
